...
const methods = JSON.parse(readFileSync('./src/methods.json'))
...
for (let m in methods) {
  ...
  let auth =
    methods[m].opts.auth === true
      ? 'required'
      : methods[m].opts.auth === 'optional'
      ? '*optional*'
      : '*none*'

  // STRING:paginated
  let pag =
    methods[m].opts.pagination === true
      ? 'paginated'
      : methods[m].opts.pagination === 'optional'
      ? '*optional*'
      : '*none*'
  ...
  // push line
  helpdoc.push(
    `| ${method} | ${auth} | ${required} | ${optional} | ${pag} | ${extended} | ${type} | ${uri} |`
  )
}

writeFileSync('./docs/available_methods.md', helpdoc.join('\n'))
