...
export default class Trakt {
  ...
  _exchange(str) {
    ...
    return got(req)
      .then((response) => {
        ...
        this._authentication.expires =
          (body.created_at + body.expires_in) * 1000

        return body
      })
      .catch((error) => {
        throw error.response && error.response.statusCode == 401
  ...
  _call(method, params) {
    if (
      method.opts['auth'] === true &&
      (!this._authentication.access_token || !this._settings.client_secret)
    )
    ...
    if (method.opts['auth'] && this._authentication.access_token)
      req.headers[
        'Authorization'
      ] = `Bearer ${this._authentication.access_token}`
